...
At the end of every task requested by the user, the AI assistant MUST self-evaluate what was learned during the execution. The assistant must consider whether any `.agents/rules/` or `.agents/skills/` need to be modified or created from scratch to prevent future mistakes or improve the efficiency of future AI agents working on this project. These changes should be implemented proactively if they are genuinely beneficial.
