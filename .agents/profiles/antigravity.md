...
*   **Documentation Maintenance**: Whenever you implement a new major feature, a significant UI change, or an architectural update, you MUST proactively update the corresponding documentation files in the `docs/` directory (across all supported languages: en, pt-BR, es) and `README` files if applicable. Do not wait for the user to remind you to document new features.
...
At the end of every task requested by the user, you MUST self-evaluate what was learned during the execution. Consider whether any `.agents/rules/` or `.agents/skills/` need to be modified or created from scratch to prevent future mistakes or improve the efficiency of future AI agents working on this project. Implement these changes proactively if they are beneficial.
