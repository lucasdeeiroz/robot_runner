...
9. **ADB Command Grouping**: Ao necessitar rodar múltiplos comandos ADB na mesma execução (ex: resgatar vários stats como meminfo, dumpsys, top), **SEMPRE** concatene os comandos no lado do Rust em um único payload enviado ao device (ex: `adb shell "cmd1 && cmd2"` ou `adb shell "cmd1 ; cmd2"`). Evite invocar o processo local do `adb.exe` dezenas de vezes seguidas para comandos triviais; isso acumula delay absurdo (IPC + USB latency) e destrói a performance do backend.