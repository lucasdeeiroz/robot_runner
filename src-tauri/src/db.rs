...
impl LogDb {
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_parent_id ON log_nodes(parent_id)",
            [],
        )?;
    ...
    pub fn get_node_ai_analysis(&self, id: &str) -> Result<Option<String>> {
        let mut stmt = self
            .conn
            .prepare("SELECT ai_analysis FROM log_nodes WHERE id = ?1")?;
    ...
    pub fn get_node(&self, id: &str) -> Result<String> {
        let mut stmt = self
            .conn
            .prepare("SELECT json_payload FROM log_nodes WHERE id = ?1")?;
