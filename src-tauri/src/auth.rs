    ...
    std::thread::spawn(move || {
        match TcpListener::bind("127.0.0.1:0") {
            Ok(listener) => {
                ...
                // Set a timeout for the whole operation (5 minutes)
                if let Ok((mut stream, _)) = listener.accept() {
                    ...
                    let _ = handle.emit("auth-error", "Nenhum código encontrado na requisição.");
                } else {
                    let _ = handle.emit("auth-error", "Tempo esgotado aguardando conexão.");
                }
            }
            Err(e) => {
