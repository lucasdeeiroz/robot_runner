...
#[tauri::command]
pub fn start_logcat(state: State<'_, LogcatState>, device: String, filter: Option<String>, level: Option<String>, output_file: Option<String>) -> Result<String, String> {
    ...
    procs.insert(device.clone(), LogcatProcess {
        child,
        buffer,
    });
...
#[tauri::command]
pub fn fetch_logcat_buffer(state: State<'_, LogcatState>, device: String) -> Result<Vec<String>, String> {
    let procs = state.0.lock().map_err(|e| e.to_string())?;
