...
export function DeviceSelector({
    ...
    const { is_test_mode } = useSettings();
    const [isDeviceDropdownOpen, setIsDeviceDropdownOpen] = useState(false);
    const containerRef = useRef<HTMLDivElement>(null);

    const isWeb = is_test_mode === 'web';

    const browserDisplayName = (id: string) => {
        switch (id) {
            case 'chrome': return 'Chrome';
            case 'firefox': return 'Firefox';
            case 'edge': return 'Edge';
            case 'headless-chrome': return 'Chrome (Headless)';
            case 'headless-firefox': return 'Firefox (Headless)';
    ...
    return (
        ...
        >
            ...
            >
                {isWeb ? (
                    <Globe size={18} className="shrink-0 text-primary mt-2 mb-2" />
                ) : (
                            ...
                            {isWeb ? (
                                (() => {
                                    const isWebBrowser = (id: string | null) => 
                                        id ? ['chrome', 'firefox', 'edge', 'headless-chrome', 'headless-firefox'].includes(id) : false;
                                    const activeBrowser = selectedDevices.find(id => isWebBrowser(id)) || 'headless-chrome';
            ...
            <AnimatePresence>
                {isDeviceDropdownOpen && (
                    ...
                    >
                        {isWeb ? (
                            <>
                                <div className="text-xs font-semibold px-2 py-1 uppercase tracking-wider">
                                    {t('run_tab.device.select_browser', 'Select Browser')}
                                </div>
                                {[
                                    { id: 'chrome', name: 'Chrome' },
                                    { id: 'firefox', name: 'Firefox' },
                                    { id: 'edge', name: 'Edge' },
                                    { id: 'headless-chrome', name: 'Chrome (Headless)' },
                                    { id: 'headless-firefox', name: 'Firefox (Headless)' }
                                ].map(b => {
                                    const isWebBrowser = (id: string | null) => 
                                        id ? ['chrome', 'firefox', 'edge', 'headless-chrome', 'headless-firefox'].includes(id) : false;
                                    const activeBrowser = selectedDevices.find(id => isWebBrowser(id)) || 'headless-chrome';
                                    const isSelected = b.id === activeBrowser;
                                    return (
                                        <StaggerItem
                                            key={b.id}
                                            className="flex items-center justify-between px-2 py-2 hover:bg-surface-variant/30 rounded-2xl group cursor-pointer"
                                            onClick={() => {
                                                toggleDevice(b.id);
                                                setIsDeviceDropdownOpen(false);
                                            }}
                                        >
                                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                                <div className={clsx(
                                                    ...
                                                    isSelected ? "bg-primary border-primary text-on-primary" : "border-outline-variant/30"
                                                )}>
                                                    {isSelected && <div className="w-2 h-2 bg-on-primary rounded-2xl" />}
                                                </div>
                                                <span className="text-sm font-medium text-on-surface/80 truncate">{b.name}</span>
                                            </div>
                                        </StaggerItem>
                                    );
                                })}
