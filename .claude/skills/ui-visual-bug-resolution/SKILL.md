...
description: Workflow autônomo e focado para diagnosticar e resolver bugs visuais e de CSS (Tailwind) relacionados ao Design System do Robot Runner. Use para "bug visual", "cor errada", "texto invisível", "hover estranho", "botão não clica". (Convertido de .agents/skills/ui-visual-bug-resolution/SKILL.md)
---
...
1. **Localização do Componente**
   - Encontre a aba ou painel afetado via `Grep`. Se o usuário der o nome de um botão, busque por chaves de tradução (ex: `t('chave')`) ou ícones do `lucide-react` atrelados a ele.
