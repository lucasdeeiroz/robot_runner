...
description: Workflow autônomo para diagnosticar problemas de conexão com dispositivos Android via ADB e reiniciar os serviços do Robot Runner. Use quando o usuário pedir para "diagnosticar adb", disser que o "dispositivo não aparece" ou que o "adb travou". (Convertido de .agents/skills/adb-diagnostics/SKILL.md)
---
...
Sempre que o usuário pedir para diagnosticar o ADB, siga estritamente os passos abaixo usando a ferramenta `Bash`:

1. **Verificar os Processos do ADB (Port Conflict)**
   - Rode: `tasklist | findstr adb.exe` (no Windows) ou `ps aux | grep adb` (Mac/Linux).
